...
当前工作树：基于 RC2 的 2026-09-21 未发布迭代；预测采纳、采购情景推演、本地模型、经营分析、角色待办与订单证据链已验
基线日期：2026-08-24
...
- 可审计经营分析：本地 Qwen 只排序闭集章节，数字、事实、风险和行动由确定性规则生成并保存快照、指纹与模型口径。
- 角色待办与订单证据链：按 BUYER/MANAGER/SUPPLIER 职责汇总只读待办；订单聚合业务记录、审计日志与库存流水形成六阶段履约时间线。
- Flyway 数据库迁移：本地默认 H2 文件库，Compose 候选使用 MySQL 8。
- 工程证据：需求、设计、API、测试、实验、论文映射、运行手册、UAT 台账和源材料留档。

本轮自动化与构建检查已通过：Spring Boot 共 57 项（业务集成 22 项、AI 故障集成 35 项）、FastAPI 23 项、真实 MySQL 8.4.11 既有 V1—V5 独立验收 164/164、前端 `vue-tsc` 与生产构建、静态与运行健康检查。本地 Qwen2.5 混合解析链固定样本回归 12/12，并已完成四角色当前工作树浏览器巡检；预测采纳、采购情景推演、经营分析、角色待办、订单证据链和账号停用后旧 JWT 立即失效均已验证。中文界面统一使用离线打包的 Noto Sans SC。新增 V6 的 MySQL 复验、用户个人 UAT 与 Docker Compose 容器启动仍未完成；执行明细见 `docs/04-testing/TEST_EXECUTION_2026-09-21.md`。
