...
_QUANTITY_WITH_UNIT_RE = re.compile(
    r"(?P<number>\d+(?:\.\d+)?|[零〇一二两三四五六七八九十百千万亿]+)\s*(?P<unit>个|件|箱|台|套|包|瓶|卷|吨|千克|公斤|kg|KG|g|克|米|升|L|升)",
)
...
def _extract_fields(intent: Intent, text: str) -> tuple[dict[str, Any], list[str]]:
    ...
    if intent == "purchase_demand":
        ...
        if dates:
            fields["required_date"] = dates[0][0]
        if any(word in text for word in ("增加", "加到", "调增", "上调")):
            fields["change_type"] = "increase"
        elif any(word in text for word in ("减少", "减到", "调减", "下调")):
        ...
        elif quantity is not None and any(word in text for word in ("调至", "改为", "改成")):
            fields["change_type"] = "set_quantity"
        elif dates and any(word in text for word in ("日期改为", "时间改为", "改到")):
            fields["change_type"] = "reschedule"
        if quantity is not None and any(word in text for word in ("调至", "改为", "改成", "加到", "减到")):
        ...
        if dates:
            ...
            if any(word in text for word in ("发货", "已发", "出库")):
                fields["ship_date"] = first_date
            fields["eta"] = dates[-1][0]
        if any(word in text for word in ("已发货", "已发", "发出")):
            fields["delivery_status"] = "shipped"
        elif any(word in text for word in ("已到货", "到达", "送达")):
...
async def _provider_result(request: ParseRequest, settings: Settings) -> RuleResult:
    system_prompt = (
        "你是供应链文本结构化解析器。只允许输出 JSON，不执行任何业务动作。"
        "intent 只能是 purchase_demand、plan_change、delivery_notice。"
        "fields 只能使用 material_code、material_name、supplier_code、supplier_name、"
        "warehouse_code、quantity、unit、required_date、plan_no、change_type、old_quantity、"
        "new_quantity、quantity_delta、order_no、ship_date、eta、tracking_no、delivery_status、reason。"
        "无法确定的字段必须为 null。"
    )
    ...
    try:
        fields = ParsedFieldValues.model_validate(raw_fields).model_dump(exclude_none=True, mode="json")
