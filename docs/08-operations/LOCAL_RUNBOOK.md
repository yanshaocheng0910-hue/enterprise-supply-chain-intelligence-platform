...
> 本手册面向 D 盘本地联调。Docker 配置只做静态检查，本机没有 Docker 时不要把 Compose 文件当作已运行证据。学校正式运维 Word 模板本轮未填写，仅保留项目内工作手册。
...
脚本会：创建 D 盘运行目录；读取根 `.env`；启动 FastAPI 8001、Spring Boot 8080 和 Vite 5173；每个后台进程使用隐藏窗口；把 stdout/stderr 写入 `.data\logs`；把准确 PID 写入 `.data\run\dev-processes.json`。前端依赖已安装时可跳过 npm 安装：
...
- `LLM_PROVIDER` 及 OpenAI-compatible 变量：未完整配置时保持 `rule`，不能伪装为大模型；
...
| AI 解析 provider 为 rule | 这是未配置完整 provider 的预期降级，不是大模型故障；如需外部 provider，补齐四项配置并重新验证 |
