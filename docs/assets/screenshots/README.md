...
| UI-23 | `23-order-timeline-desktop.png` | `buyer` / 订单详情与履约追踪 1440px | 完成订单六阶段与 17 条真实业务/审计/库存证据可见 |
| UI-24 | `24-order-timeline-mobile.png` | `buyer` / 订单详情与履约追踪 390px | 抽屉宽度 390/390，闭环阶段、时间线和证据来源在窄屏可读 |

UI-01—UI-07 仍只证明 RC2 当时状态；UI-08—UI-10 是 2026-09-20 证据；UI-11—UI-24 是 2026-09-21 本地模型、字体、四角色、经营分析、待办和履约证据链当前工作树证据。对应巡检控制台为 `0 errors / 0 warnings`。本索引证明内部候选版巡检，不代表用户个人 UAT 已通过。
